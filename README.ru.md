...
- Проверьте, что локальные/служебные файлы игнорируются (`.vs`, `artifacts_build`, `bin`, `obj`, логи)
- Проверьте, что в отслеживаемых конфигурациях нет секретов
- Если служебные файлы уже были добавлены в Git до `.gitignore`, уберите их из индекса один раз:
...
Перед публичным релизом добавьте файл лицензии (`LICENSE`).
